...
public class PoeNinjaWrapper : IPriceFetcher
{
    ...
    public async Task<SearchItemGroup> GetFragmentData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = fragments.Select(detail => new SearchItem { Name = detail.currencyTypeName, Price = Math.Round(detail.chaosEquivalent), Rarity = ItemRarity.Normal, Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetDivinationCardData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = divinationCards.Select(detail => new SearchItem { Name = detail.name, Price = Math.Round(detail.chaosValue), Rarity = ItemRarity.DivinationCard, Volatile = detail.count < 10 || detail.sparkline.Volatile(), Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueMapData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = uniqueMaps.Select(detail => new SearchItem { Name = detail.name, Price = Math.Round(detail.chaosValue), Rarity = detail.detailsId.Contains("relic") ? ItemRarity.UniqueFoil : ItemRarity.Unique, MapTier = detail.mapTier!.Value, Volatile = detail.count < 10 || detail.sparkline.Volatile(), Source = Name, AllowCorrupted = true }).Where(i => i.Rarity != ItemRarity.UniqueFoil).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueJewelData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = uniqueJewels.Select(detail => new SearchItem { Name = detail.name, Price = Math.Round(detail.chaosValue), Rarity = detail.detailsId.Contains("relic") ? ItemRarity.UniqueFoil : ItemRarity.Unique, BaseType = detail.baseType, Variant = detail.variant, Volatile = detail.count < 10 || detail.sparkline.Volatile(), Source = Name }).Where(i => i.Rarity != ItemRarity.UniqueFoil).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueWeaponData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = uniqueWeapons.Select(detail => new SearchItem { Name = detail.name, Price = Math.Round(detail.chaosValue), Links = (detail.links.HasValue && detail.links.Value > 4) ? detail.links.Value : 0, Rarity = detail.detailsId.Contains("relic") ? ItemRarity.UniqueFoil : ItemRarity.Unique, BaseType = detail.baseType, Variant = detail.variant, Volatile = detail.count < 10 || detail.sparkline.Volatile(), Source = Name }).Where(i => i.Rarity != ItemRarity.UniqueFoil).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
