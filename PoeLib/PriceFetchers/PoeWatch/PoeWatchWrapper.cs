...
public class PoeWatchWrapper : IPriceFetcher
{
    ...
    private IEnumerable<CurrencyPrice> GetPrices(CurrencyData[] currencyData)
    {
        foreach (var currencyType in (TradeCurrencyType[]) Enum.GetValues(typeof(TradeCurrencyType)))
        {
            switch (currencyType)
            {
                case TradeCurrencyType.Unknown:
                    continue;
                case TradeCurrencyType.Chaos:
                    yield return new CurrencyPrice {Type = TradeCurrencyType.Chaos};
                    break;
                default:
                {
    ...
    public async Task<SearchItemGroup> GetFragmentData(string league)
    {
        ...
        try
        {
            ...
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Normal, Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetDivinationCardData(string league)
    {
        ...
        try
        {
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.DivinationCard, Volatile = detail.lowConfidence, Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueMapData(string league)
    {
        ...
        try
        {
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Unique, MapTier = detail.mapTier ?? 1, Volatile = detail.lowConfidence, Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueJewelData(string league)
    {
        ...
        { 
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Unique, Variant = string.Empty, Volatile = detail.lowConfidence, Source = Name }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueWeaponData(string league)
    {
        ...
        { 
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Unique, Links = detail.linkCount > 4 ? detail.linkCount.Value : 0, Volatile = detail.lowConfidence, Source = Name }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetUniqueAccessoryData(string league)
    {
        ...
        try
        {
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Unique, Volatile = detail.lowConfidence, Source = Name }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
    ...
    public async Task<SearchItemGroup> GetFossils(string league)
    {
        ...
        try
        {
            ...
            if (itemData == null)
                return searchItemGroup;
            var items = itemData.OrderByDescending(f => f.min);
            searchItemGroup.SearchItems = items.Select(detail => new SearchItem {Name = detail.name, Price = Math.Round(detail.min), Rarity = ItemRarity.Currency, Volatile = detail.lowConfidence, Source = Name, AllowCorrupted = true }).ToList();
            return searchItemGroup;
        }
        catch (Exception ex)
        {
