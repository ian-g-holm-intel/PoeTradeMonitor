...
public class QuicksilverFlaskCrafter : CrafterBase
{
    public QuicksilverFlaskCrafter(IPoeHudWrapper phw, ITradeCommands tc, IRarityStateMachine rsm) : base(phw, tc, rsm)
    {
    }
    ...
    public override async Task Craft()
    {
        ...
        try
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (HasCurrency(TradeCurrencyType.Aug) && (HasAssassinsMark && GetNumberOfPrefixes() == 0) || ((HasLife || HasPhys) && GetNumberOfSuffixes() == 0))
