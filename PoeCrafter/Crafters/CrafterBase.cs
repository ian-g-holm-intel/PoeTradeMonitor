...
public abstract class CrafterBase : ICrafter
{
    ...
    private TradeCurrencyType currencySpam = TradeCurrencyType.Unknown;
    ...
    protected async Task UseCurrency(TradeCurrencyType type)
    {
        if (!HasCurrency(type))
    ...
    protected async Task UseEssence(TradeCurrencyType type)
    {
        if (!HasEssence(type))
            throw new CurrencyNotFoundException();
    ...
    protected async Task StartUsingCurrency(TradeCurrencyType type)
    {
        currencySpam = type;
    ...
    protected async Task StartUsingEssence(TradeCurrencyType type)
    {
        currencySpam = type;
    ...
    {
        currencySpam = TradeCurrencyType.Unknown;
        var itemLocation = GetItemLocation();
    ...
    private async Task CurrencySpamCheck(Func<Task> action)
    {
        ...
        await action();
        if (tempCurrencySpam != TradeCurrencyType.Unknown)
    ...
    private Point GetCurrencyLocation(TradeCurrencyType type)
    {
    ...
    private Point GetEssenceLocation(TradeCurrencyType type)
    {
    ...
    protected bool HasCurrency(TradeCurrencyType type)
    {
        try
        {
            for (int i = 0; i < 5; i++)
    ...
    protected decimal GetCurrencyCount(TradeCurrencyType type)
    {
        try
        {
    ...
    protected bool HasEssence(TradeCurrencyType type)
    {
        try
        {
            for (int i = 0; i < 5; i++)
    ...
    protected decimal GetEssenceCount(TradeCurrencyType type)
    {
        try
        {
    ...
    protected bool HasPathToRare => HasCurrency(TradeCurrencyType.Alch) || HasEssences || HasCurrency(TradeCurrencyType.Transmute) && HasCurrency(TradeCurrencyType.Regal);
    ...
    protected virtual ModValue[] GetCraftingMods()
    {
        return poeHud.CraftingSlotMods;
    }

    protected virtual ModValue[] GetEssenceCraftingMods()
    {
    ...
    protected virtual bool HasMods(Predicate<ModValue[]> predicate)
    {
    ...
    protected async Task MakeRare()
    {
        ...
        {   
            if (HasCurrency(TradeCurrencyType.Transmute) && HasCurrency(TradeCurrencyType.Regal))
            {
        ...
        {
            if (HasCurrency(TradeCurrencyType.Regal))
            {
                await stateMachine.ChangeState(Trigger.Regal);
            }
            else
    ...
    protected async Task MakeMagic()
    {
        if (poeHud.CraftingSlotRarity == ItemRarity.Normal)
        {
            if (HasCurrency(TradeCurrencyType.Transmute))
            {
                await stateMachine.ChangeState(Trigger.Transmute);
            }
            else
    ...
    {
        if (HasCurrency(TradeCurrencyType.Scour))
