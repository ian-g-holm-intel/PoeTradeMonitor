...
public interface IPoeHttpClient
{
    ...
    Task<HttpResponseMessage> PostSearchRequest(string league, TradeSearchRequest searchRequest);
...
public class PoeHttpClient : IPoeHttpClient
{
    ...
    {
        var accountName = Environment.GetEnvironmentVariable("POE_ACCOUNT_NAME") ?? throw new InvalidOperationException("POE_ACCOUNT_NAME environment variable is required");
        var client = httpClientFactory.CreateClient("PoeApi");
        var message = new HttpRequestMessage(HttpMethod.Get, $"/character-window/get-stash-items?league={league}&tabIndex={tabIndex}&accountName={accountName}");
        AddApiHeaders(message);
        using (await asyncLock.LockAsync().ConfigureAwait(false))
        {
            return await client.SendAsync(message).ConfigureAwait(false);
    ...
    public async Task<HttpResponseMessage> PostSearchRequest(string league, TradeSearchRequest searchRequest)
    {
    ...
    public async Task SendTradeWhisper(string searchId, string itemId, int count)
    {
        ...
        var tradeSearchResponse = await fetchResponse.Content.ReadFromJsonAsync<TradeFetchResponse>().ConfigureAwait(false);

        var itemSearchResult = tradeSearchResponse?.Result.First();
        ...
        var whisper_token = itemSearchResult.Listing.WhisperToken;
    ...
    {
        var itemWhisper = new ItemWhisperRequest { Token = whisper_token };
        if (count > 0)
    ...
    private ClientWebSocket GetWebSocketClient(Uri requestUri)
    {
        ...
        client.Options.SetRequestHeader("user-agent", Constants.UserAgent);
    ...
    private HttpRequestMessage AddApiHeaders(HttpRequestMessage message)
    {
        ...
        message.Headers.Add("Sec-Ch-Ua", Constants.SecChUa);
        message.Headers.Add("Sec-Ch-Ua.Arch", "\"x86\"");
        message.Headers.Add("Sec-Ch-Ua.Bitness", "\"64\"");
        message.Headers.Add("Sec-Ch-Ua-Full-Version-List", Constants.SecChUaFullVersionList);
