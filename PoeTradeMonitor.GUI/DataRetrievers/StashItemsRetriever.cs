...
{
    Task<TradeItem[]> GetStashItems(string League);
}
...
public class StashItemsRetriever : IStashItemsRetriever
{
    ...
    public async Task<TradeItem[]> GetStashItems(string League)
    {
        try
        {
            var tabIndex = 0;
            ...
            var stashItems = await JsonSerializer.DeserializeAsync<StashTabResponse>(jsonStream);
            return stashItems?.Items.ToArray() ?? Array.Empty<TradeItem>();
        }
        catch(Exception ex)
        {
