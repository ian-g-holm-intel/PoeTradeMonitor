                ...
                try
                {
                    while (!ctSource.Token.IsCancellationRequested)
                    {
                        if (!tradeBot.IsExecutingTrade && !poeHud.PlayerInParty() && poeHud.PartyInvites.Any())
                        {
                            ...
                            var invite = poeHud.PartyInvites.OrderBy(invite => charactersWaitList.Keys.SingleOrDefault(request => request.CharacterName == invite.Name || request.AccountName == invite.AccountName)?.Value ?? 0).FirstOrDefault();
                            if(invite != null)
                            {
                                var accountName = invite.AccountName;
                                var characterName = invite.Name;
                                
                                await tradeCommands.LeftClickMouse(invite.AcceptButton.Center);
                                await Task.Delay(500);
                                ...
                                if (request != null)
                                {
                                    await callback.JoinedPartyAsync(request.AccountName, request.AccountName == invite.AccountName ? invite.Name : request.CharacterName);
