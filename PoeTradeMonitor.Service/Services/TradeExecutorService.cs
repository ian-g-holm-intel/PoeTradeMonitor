...
using PoeTradeMonitor.Service.Common;
using PoeLib.Common;
...
public class TradeExecutorService : ITradeExecutorService
{
    ...
    private readonly TradeBotService tradeBot;
    ...
    public TradeExecutorService(ITradeCommands tc, ITradeBotStateMachine sm, IPoeChatWatcher cw, INotificationClient nc, ILogger<TradeExecutorService> log, TradeBotService tradeBot)
    {
        tradeCommands = tc;
        stateMachine = sm;
    ...
    public async Task ExecuteItemTrade(ItemTradeRequest tradeRequest, CancellationToken ct = default)
    {
        try
        {
            ...
            try
            {
                ...
                await tradeCommands.WithdrawCurrency(tradeRequest.Currencies);
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    try
                    {
                        ...
                        await tradeCommands.RemoveInventoryCurrency(tradeRequest.Currencies, true, false);
        ...
        catch (InvalidOperationException ex)
        {
            ...
            var characterMessage = new PoeLib.Proto.CharacterMessage { Character = "Me", Message = "Fatal Bot Error", Source = PoeLib.Proto.CharacterMessage.Types.MessageSource.Me };
            await tradeBot.AddCharacterMessage(new PoeLib.Proto.AddCharacterMessageRequest { Message = characterMessage }, null!);
        }
    }
}
