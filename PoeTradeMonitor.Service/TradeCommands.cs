...
public interface ITradeCommands
{
    ...
    Task RemoveInventoryCurrency(IEnumerable<CurrencyInfo>? currencyToRemove = null, bool isTrade = false, bool selectTab = true);
    Task RemoveInventoryItems(bool shouldDelay = false);
    Task WithdrawCurrency(IEnumerable<CurrencyInfo> currencies);
    Task AntiAFK();
    Task CloseAllPanels();
...
public class TradeCommands : ITradeCommands
{
    ...
    public async Task RemoveInventoryCurrency(IEnumerable<CurrencyInfo>? currencyToRemove = null, bool isTrade = false, bool selectTab = true)
    {
    ...
    private async Task CheckForPartyInvite(bool shouldDecline, int inventorySlotCount)
    {
        ...
        foreach (var invite in poeHud.PartyInvites)
        {
            if (shouldDecline)
            {
                await LeftClickMouse(invite.DeclineButton.Center);
            }
            else
            {
                await LeftClickMouse(invite.AcceptButton.Center);
            }
        }
    }
    ...
    private async Task MoveItem(ServerInventory.InventSlotItem item, bool shouldDelay, bool shiftClick = true)
    {
        await HoldKey(VirtualKeyCode.CONTROL, async () =>
        {
            if (shiftClick)
                await SendKeyDown(VirtualKeyCode.SHIFT);
            await LeftClickMouse(item.GetCenter(), shouldDelay);
            if (shiftClick)
    ...
    public async Task WithdrawCurrency(IEnumerable<CurrencyInfo> currencies)
    {
        await SelectTab("$");
    ...
    {
        ActivatePoe();
        if (!await CheckConditionTrue(() => poeHud.AreaName == hideoutName, 100, 20, ct))
    ...
    {
        var tradeInvite = poeHud.TradeInvites.SingleOrDefault(invite => invite.Name.Equals(characterName, StringComparison.OrdinalIgnoreCase));
        if (tradeInvite != null)
        {
            await Task.Delay(1000);
            await LeftClickMouse(tradeInvite.AcceptButton.Center);
            return;
        }
        else
        {
        ...
        return Task.Run(async () =>
        {
            using (await inputLock.LockAsync())
            {
                ...
                while (!ct.IsCancellationRequested && stopwatch.ElapsedMilliseconds < timeoutMs)
                {
                    ...
                    var currentAcceptButtonLocation = poeHud.TradeInvites.SingleOrDefault(invite => invite.Name.Equals(characterName, StringComparison.OrdinalIgnoreCase))?.AcceptButton.Center ?? new Point(-1, -1);
    ...
    public async Task<bool> CheckItemReceived(ItemTradeRequest tradeRequest)
    {
        ...
        try
        {
            ...
            var expectedStackSize = tradeRequest.Item.StackSize;
            ...
            foreach (var invItem in poeHud.PlayerInventoryItems)
            {
                ...
                await DoUntilTrueAsync(() =>
                {
                    ...
                    if (itemElement != null && itemElement.Center != Point.Empty)
            ...
            if(totalItemCount == expectedStackSize)
            { 
                return true;
            }
            else
    ...
    public async Task<bool> VerifyItemsCorrect(ItemTradeRequest tradeRequest, IEnumerable<NormalInventoryItem> theirTradeItems)
    {
        ...
        if(totalItemCount != expectedStackSize)
        {
            log.LogError($"Expected stack size: {expectedStackSize}, Actual stack size: {totalItemCount}");
            return false;
        }
        return true;
    }
    ...
    {
        while (poeHud.TheirTradeItems.Any(i => !i.IsSaturated))
        {
            foreach (var item in poeHud.TheirTradeItems.Where(i => !i.IsSaturated))
            {
                if (item == null) return;
                MouseMove(item.Center);
                await Task.Delay(20);
