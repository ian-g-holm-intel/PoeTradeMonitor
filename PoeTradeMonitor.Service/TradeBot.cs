...
public class TradeBot : ITradeBot
{
    ...
    public Task StartAsync(CancellationToken cancellationToken)
    {
        ...
        tradeLoopTask = Task.Factory.StartNew(async () =>
        {
            try
            {
                ...
                {
                    if (DequeueMostValuable(out var tradeRequest) && tradeRequest != null)
                    {
    ...
    private bool DequeueMostValuable(out ItemTradeRequest? tradeRequest)
    {
        ...
        lock (queueLock)
        {
            tradeRequest = itemTradeQueue.OrderByDescending(item => item.Price.PriceInBaseCurrency(item.DivineRate)).First();
